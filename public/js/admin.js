...
async function init() {
  ...
  fetchUsers();
  fetchPhotos();
  displayWelcomeMessage(token); // ✅ Show welcome message
}
...
function displayWelcomeMessage(token) {
  try {
    const payload = JSON.parse(atob(token.split('.')[1]));
    ...
    if (welcomeDiv) {
      welcomeDiv.textContent = `Welcome, ${escapeHtml(name)}!`;
    }
  } catch (err) {
    console.error('Error parsing token for welcome message:', err);
  }
}
