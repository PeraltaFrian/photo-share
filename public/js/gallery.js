...
async function fetchNewToken() {
  try {
    ...
    // Save token to localStorage so init() picks it up
...
async function init() {
  ...
  // Check if admin and set up button (existing)
  try {
    const payload = JSON.parse(atob(token.split('.')[1]));
  ...
  // Fetch and display user info in UI
...
// Show only welcome message (no logout button here)
async function displayUserInfo() {
  try {
    ...
    welcomeDiv.textContent = `Welcome, ${escapeHtml(nameToShow)}!`;
  } catch (err) {
...
// Escaping function to prevent XSS in username/email output
...
function setupUI() {
  // Removed logoutBtn listener here because it's handled in navbar
